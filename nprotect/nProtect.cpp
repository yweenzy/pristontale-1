...
int nP::findNPMON()
{
    ...
    if (hwnd != 0) {
		 //MessageBox(NULL,"Already execute","OK",MB_OK);	// 실행중
		 return 0;
	
    } else {
		//MessageBox(NULL,"not execute","OK",MB_OK);		// 비실행중
		return 1;
   }
...
BOOL nP::CheckCRC(char * FilePath)
{
	...
	{
		//MessageBox(NULL,"npchk.dll File not find.","error",MB_OK);	// npchk.dll 화일이 없다
		return FALSE;
	}
	...
	{
		//MessageBox(NULL,"File is alteration.","error",MB_OK);		// 파일이 변조 됬다
		return FALSE;
	}
	return TRUE;
}
...
BOOL nP::NPGetWindowsVersion()
{
	...
	if (Result == FALSE)
		return FALSE; // 윈도우 9x 계열

	if (VersionInfo.dwPlatformId == VER_PLATFORM_WIN32_NT)
		IsNT = TRUE; // 윈도우 NT 계열
	else
		IsNT = FALSE; // 윈도우 9x 계열

	return TRUE;	// 윈도우 NT 계열
}
...
BOOL nP::NPFindSICEProtect()
{
	...
	if(hInstanceNPFSICE != NULL)
	{
		...
		{
			if(pnPFindSICE())
				return FALSE;	// SoftIce Detect 소프트아이스 발견
			else
				return TRUE;	// SoftICE Not Detect 소프트아이스 발견 안됨
		}
	}		
