...
void cASSAPARTSHOT::Draw(POINT3D *pPosi , POINT3D *pAngle)
{
	...
	// Turn on Z-buffering
	//renderDevice.SetRenderState( D3DRENDERSTATE_ZENABLE, TRUE ); (문제가 생기는거 같아서 빼줬다)
...
void AssaShelltomUpWeapon2::Main()
{
 	...
 	if(pChar->HvRightHand.dwItemCode != 0)
	{
		...
		else if((pChar->HvRightHand.dwItemCode & sinITEM_MASK2) ==  sinWM1)//wand.
		{
    		StartZPos = 0;
			WeaponPosCount = WeaponSize/1000;
			GlowWidth = 35*fONE;
		...
		else if((pChar->HvRightHand.dwItemCode & sinITEM_MASK2) ==  sinWS1)//Bow shooting
		{
		...
		else if((pChar->HvRightHand.dwItemCode & sinITEM_MASK2) ==  sinWH1)//Hammer
		{
  			StartZPos = 0;
			Width = 12*fONE;
			WeaponPosCount = WeaponSize/1000;
		...
		else if((pChar->HvRightHand.dwItemCode & sinITEM_MASK2) ==  sinWA1)//Axe
		{
 			StartZPos = 0;
			Width = 12*fONE;
			WeaponPosCount = WeaponSize/1000;
		...
		//DRZ_EDIT (Add Weapon Handling for NewClassWeapon here) 
		else
 		{
			StartZPos = 0;
			WeaponSize = pChar->HvRightHand.SizeMax;
