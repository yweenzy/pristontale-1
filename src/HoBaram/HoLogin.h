...
public:
	...
	int tribeNum;			//종족번호
	RECT menuPos[2];		//메뉴셋팅.
	//int						MoryonJobLogoImage[CHARACTER_JOB_MAX];	//TempscronJobLogo Image 번호..

							//모라연 얼굴그림.
	int						MorFaceImage[CHARACTER_JOB_MAX][4][2];	//0:Fighter 1:Mechanician 2:Archer 3: Pikeman //얼굴의 종류.. //0:옆얼굴 1: 정면얼굴.

	void Init_UseMor();	//0 : 모라이언선택안됨. 1:모라이언선택됨.
...
private:
	...
	//int						TempscronJobLogoImage[CHARACTER_JOB_MAX];	//TempscronJobLogo Image 번호..

	

	//Face Select 2d 얼굴 이미지들...
	DIRECTDRAWSURFACE	FaceSelectImage;
	int						TempFaceImage[CHARACTER_JOB_MAX][4][2];	//0:Fighter 1:Mechanician 2:Archer 3: Pikeman //얼굴의 종류.. //0:옆얼굴 1: 정면얼굴.
