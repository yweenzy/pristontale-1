...
void HoEffectEventController::Init(HoEffectModelInterface *model)
{
	...
	*/
	...
	//DRZ_EDIT try to fix crash
	//if (m_EffectEvents.size() > 0)
	//{
	for (iter = m_EffectEvents.begin(); iter != m_EffectEvents.end() && !(*iter)->GetStartTime(); iter++)
	{
		(*iter)->VisitEffectModel(*model);
	}
	//}
