...
int smSTAGE3D::MakeAreaFaceList(int x, int z, int width, int height, int top, int bottom)
{
	...
	for (cntX = sx; cntX < wx; cntX++)
	{
		for (cntZ = sz; cntZ < wz; cntZ++)
		{
			...
			if (StageArea[lx][lz]) { 							//Domain
				num = StageArea[lx][lz][0];						//Be included in the zone with Facelist
...
int	smSTAGE3D::LoadFile( char *szFile )
{
	...
	if (!hFile)
	{
		Utils_Log(LOG_ERROR, "Cannot open StageFile(%s)", szFile);
		return FALSE;
	}

	//Retrieves the contents of the header
	int size = ReadFile(hFile, &FileHeader, sizeof(smDFILE_HEADER), &dwAcess, NULL);

	//Header is wrong (wrong or this version.)
	if ( lstrcmp( FileHeader.szHeader , szSMDFileHeader )!=0 ) 
	{
		Utils_Log(LOG_ERROR, "Wrong StageFile Header (%s)", szFile);
		CloseHandle( hFile );
		return FALSE;
	}

	//Reading the contents of the class
	ReadFile( hFile , &Head, sizeof( smSTAGE3D ), &dwAcess , NULL );
	smTEXLINK *lpOldTexLink = TexLink;
	
	/*
#ifdef _DEBUG
	Utils_Log(LOG_DEBUG, "smSTAGE3D::LoadFile: %s", szFile);
	Utils_Log(LOG_DEBUG, "MatCount: %d", FileHeader.MatCounter);
	Utils_Log(LOG_DEBUG, "Vertex: %d", nVertex);
	Utils_Log(LOG_DEBUG, "Face: %d", nFace);
	Utils_Log(LOG_DEBUG, "TexLink: %d", nTexLink);
	Utils_Log(LOG_DEBUG, "nLight: %d", nLight);
#endif
	*/
	...
	//Texture coordinates, so a new calibration must be a pointer to an address
	...
	//Read the data from the zone area
